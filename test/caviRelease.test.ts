...
const SCRIPT = path.resolve("scripts/cavi-release.mjs");
...
describe("CAVI release facts", () => {
  ...
  it("publishes and dispatches the verified CAVI artifact on Node 24", async () => {
    const workflow = await readFile(".github/workflows/release.yml", "utf8");
    for (const marker of [
      "workflow_dispatch:",
      "node-version: 24.18.1",
      "companion-for-claude-cavi-release-${TAG}.tar.gz",
      "cavi-release.json",
      "CONSUMER_DISPATCH_TOKEN",
      "/repos/cavi-ai/cavi-home/dispatches",
      "cavi-oss-release",
    ]) {
